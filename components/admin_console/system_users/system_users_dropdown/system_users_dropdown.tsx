...
> {
  ...
  // Datajoy: this is where the client triggers a network request
  ...
  handleRevokeSessions = async () => {
    ...
    const { data, error } = await this.props.actions.revokeAllSessionsForUser(
      this.props.user.id
    )
  ...
  render () {
    ...
    let currentRoles = (
      <FormattedMessage id='admin.user_item.member' defaultMessage='Member' />
    )
